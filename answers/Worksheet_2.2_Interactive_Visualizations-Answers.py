...
# You will be able to filter the bar chart by industry with a dropdown.
...
# We first have to create a dropdown with a list of the industries.  You can read the docs for the
# selectbox here: https://docs.streamlit.io/develop/api-reference/widgets/st.selectbox
# Don't forget to assign the value from the selectbox to a variable.
industry_list = raw_data['industry'].unique()
selected_industry = st.selectbox("Select industry", industry_list)

# Now create two tabs, one for the data table, and another for a chart.  You may create more if you wish.
...
with chart_tab:
    chart_data = (
        raw_data[raw_data['industry'] == selected_industry]
        .groupby('botfam', as_index=False)['hosts']
        .sum()
